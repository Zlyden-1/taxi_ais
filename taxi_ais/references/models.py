...
class Client(models.Model):
    ...
    organisation = models.CharField(
        max_length=150, db_index=True, null=False, blank=False, verbose_name="Наименование организации"
    )
...
class VehicleHistory(models.Model):
    driver = models.ForeignKey(to="Driver", on_delete=models.CASCADE, null=False, blank=False, verbose_name="Водитель")
    vehicle = models.ForeignKey(to="Vehicle", on_delete=models.CASCADE, null=False, blank=False, verbose_name="ТС")
...
class Vehicle(models.Model):
    ...
    license_plate = models.CharField(max_length=10, verbose_name="Гос. номер")
    ...
    vehicle_type = models.ForeignKey(to="VehicleType", on_delete=models.PROTECT, verbose_name="Тип ТС")
    manufacture_year = models.IntegerField(null=True, blank=True, verbose_name="Год выпуска")
    registration_certificate_scan = models.FileField(
        upload_to="vehicles/registration_certificates/", null=True, blank=True, verbose_name="Скан СТС"
    )
    vehicle_passport_scan = models.FileField(
        upload_to="vehicles/vehicle_passports/", null=True, blank=True, verbose_name="Скан ПТС"
    )
    status = models.ForeignKey(
        to="VehicleStatus", on_delete=models.SET_NULL, null=True, blank=False, verbose_name="Статус"
    )
    location = models.ForeignKey(
        to="VehicleLocation", on_delete=models.SET_NULL, null=True, blank=False, verbose_name="Место базирования"
    )
    rent_type = models.CharField(
        max_length=1, choices=rent_type_choices, null=True, blank=False, verbose_name="Способ использования"
    )
    driver = models.OneToOneField(
        to=Driver, on_delete=models.SET(""), related_name="vehicle", null=True, blank=True, verbose_name="Водитель"
    )
    usage_history = models.ManyToManyField(
        to=Driver, through=VehicleHistory, related_name="vehicles_history", verbose_name="История использования"
    )
    acceptance_certificate_scan = models.FileField(
        upload_to="vehicles/acceptance_certificates/", null=True, blank=True, verbose_name="Акт приема-передачи"
    )
    leasing_contract_scan = models.FileField(
        upload_to="vehicles/leasing_contracts/", null=True, blank=True, verbose_name="Договор лизинга"
    )
    ...
    def delete(self, *args, **kwargs):
        if self.registration_certificate_scan:
            rc_path = os.path.join(
                settings.MEDIA_ROOT, f"vehicles/registration_certificates/{self.registration_certificate_scan.name}"
            )
            os.remove(rc_path)
        if self.vehicle_passport_scan:
            vp_path = os.path.join(settings.MEDIA_ROOT, f"vehicles/vehicle_passports/{self.vehicle_passport_scan.name}")
            os.remove(vp_path)
        super().delete(*args, **kwargs)
...
class VehicleType(models.Model):
    ...
    rent_price = models.IntegerField(null=False, blank=False, verbose_name="Аренда")

    def __str__(self):
        str_ = f"{self.brand} {self.model}"
        if self.has_gas_cylinder_equipment:
            str_ = f"{str_} с ГБО"
        return str_
